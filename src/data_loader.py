...
def training_load():
    with gzip.open('../data/train-images-idx3-ubyte.gz','rb') as fp:
        header=fp.read(16)
    ...
    with gzip.open('../data/train-labels-idx1-ubyte.gz','rb') as fp:
        header=fp.read(8)
...
def test_load():
    with gzip.open('../data/t10k-images-idx3-ubyte.gz','rb') as fp:
        header=fp.read(16)
        mgn,nimg,nrow,ncol=struct.unpack('>llll',header)
        data=np.fromstring(fp.read(),dtype='uint8')
    fp.close()
    test_img=np.reshape(data.astype(float)/255,(nimg,ncol*nrow))
    with gzip.open('../data/t10k-labels-idx1-ubyte.gz','rb') as fp:
        header=fp.read(8)
        mgn,nimg=struct.unpack('>ll',header)
        data=np.fromstring(fp.read(),dtype='uint8')
    fp.close()
